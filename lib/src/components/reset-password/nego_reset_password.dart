            ...
            builder: (context, value, child) {
              ...
              return Column(
                children: [
                  ...
                  Column(
                    children: [
                      CheckListTile(isChecked: isMinLength, text: NegoLocalizations.of(context)?.resetPasswordMin6 ?? 'Minimum 6 characters'),
                      const SizedBox(height: 15),
                      CheckListTile(isChecked: isLetter, text: NegoLocalizations.of(context)?.resetPassword1Letter ?? 'A capital letter'),
                      const SizedBox(height: 15),
                      CheckListTile(isChecked: isSymbol, text: NegoLocalizations.of(context)?.resetPassword1Symbol ?? 'At least 1 symbol'),
                      const SizedBox(height: 15),
                      CheckListTile(isChecked: isNumber, text: NegoLocalizations.of(context)?.resetPassword1Number ?? 'At least 1 number'),
                    ],
                  ),
                ],
              );
          ...
          ValueListenableBuilder(
              ...
              builder: (context, value, child) {
                ...
                return Column(
                  children: [
                    const SizedBox(height: 15),
                    CheckListTile(isChecked: isEqualConfirm, text: NegoLocalizations.of(context)?.resetPasswordEquals ?? 'Passwords are the same'),
                    const SizedBox(height: 20),
                    BaseContainerButton(
